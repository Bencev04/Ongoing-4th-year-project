...
class TestListJobs:
    ...
    def test_list_jobs_scoped_to_tenant(
        ...
        mock_get.return_value = {"items": [], "total": 0, "page": 1, "per_page": 100, "pages": 0}
...
class TestCreateJob:
    ...
    def test_create_job_rejects_wrong_tenant_customer(
        self, mock_cust: AsyncMock, owner_client: TestClient,
    ) -> None:
...
class TestGetJob:
    ...
    def test_get_job_wrong_tenant_denied(
        self, mock_job: AsyncMock, owner_client: TestClient, sample_job: dict,
    ) -> None:
...
class TestDeleteJob:
    ...
    def test_employee_cannot_delete(
        self, mock_get: AsyncMock, employee_client: TestClient, sample_job: dict,
    ) -> None:
        mock_get.return_value = sample_job
...
class TestJobQueue:
    ...
    def test_queue_returns_unscheduled(
        ...
        mock_get.return_value = [
            {
                "id": 3, "title": "Pending Job", "customer_id": 10,
                "owner_id": 1, "status": "pending", "priority": "normal",
...
class TestTenantIsolationAndSecurity:
    ...
    def test_cannot_access_other_tenant_job(
        self, mock_get: AsyncMock, owner_client: TestClient, sample_job: dict,
    ) -> None:
        """
...
class TestSchedulingConflictDetection:
    ...
    def test_detect_overlapping_time_conflict(
        self, mock_assignee: AsyncMock, mock_get: AsyncMock, owner_client: TestClient,
    ) -> None:
        """
...
class TestCalendarEndpoint:
    ...
    def test_calendar_returns_200_with_grouped_days(
        self, mock_cal: AsyncMock, owner_client: TestClient,
    ) -> None:
        """
...
class TestAssignEndpoint:
    ...
    def test_assign_happy_path_no_times(
        ...
        Verifies:
        - 200 status code
        - update_job called with assigned_to
        """
        mock_get.return_value = sample_job
        mock_update.return_value = {**sample_job, "assigned_to": 7}
    ...
    def test_assign_with_times_no_conflict(
        ...
        mock_get.return_value = sample_job
        mock_assignee.return_value = []  # No existing jobs
        mock_update.return_value = {**sample_job, "assigned_to": 7, "status": "scheduled"}
