...
INTER_REQUEST_DELAY = 0.15  # seconds — proactive rate-limit avoidance
...
class RetryTransport(httpx.BaseTransport):
    """
    HTTP transport with automatic retry on 429 (Too Many Requests).
...
def _find_org_by_slug(db: httpx.Client, slug: str) -> Optional[int]:
...
def _find_user_by_email(db: httpx.Client, email: str) -> Optional[int]:
...
def _provision_test_data(gw: httpx.Client) -> None:
    ...
    try:
        ...
        owner_id = _ensure_user(
            db, OWNER_EMAIL, "Demo", "Owner",
            OWNER_PASSWORD, "owner", None, co1_id, org1_id,
        )
        ...
        for email, fn, ln, role in [
            ...
            _ensure_user(
                db, email, fn, ln,
                OWNER_PASSWORD, role, owner_id, co1_id, org1_id,
            )

        # ── Owner (tenant 2 — cross-tenant isolation) ─────────────────────
        owner2_id = _ensure_user(
            db, OWNER2_EMAIL, "Second", "Owner",
            OWNER2_PASSWORD, "owner", None, co2_id, org2_id,
        )
...
def pytest_configure(config):
    ...
    try:
        ...
        if resp.status_code != 200:
            pytest.exit(
                f"Stack health check failed: {resp.status_code}. "
