...
class TestHealthChecks:
    ...
    def test_frontend_serves_login_page(
    ...
    def test_full_workflow(
        ...
        try:
            ...
            assert cust_resp.status_code in (200, 201), (
                f"Customer creation failed: {cust_resp.status_code} "
                f"— {cust_resp.text}"
            )
            customer = cust_resp.json()
            created_customer_id = (
                customer.get("id") or customer.get("customer_id")
            )
            ...
            assert job_resp.status_code in (200, 201), (
                f"Job creation failed: {job_resp.status_code} "
                f"— {job_resp.text}"
            )
            job = job_resp.json()
...
class TestNginxRouting:
    ...
    def test_jobs_route(
        ...
        resp = http_client.get(
            "/api/v1/jobs/", headers=owner_headers
        )
    ...
    def test_customers_route(
        ...
        resp = http_client.get(
            "/api/v1/customers/", headers=owner_headers
        )
    ...
    def test_internal_routes_blocked(
        self, http_client: httpx.Client
    ) -> None:
        """
