...
class TestCustomerIsolation:
    ...
    def test_tenant2_cannot_see_tenant1_customers(
        ...
        try:
            ...
            ids = [
                c.get("id") or c.get("customer_id") for c in customers
            ]
...
class TestUserIsolation:
    ...
    def test_tenant2_cannot_see_tenant1_employees(
        ...
        t1_employees = t1_data if isinstance(t1_data, list) else (
            t1_data.get("data")
        ...
        t2_employees = t2_data if isinstance(t2_data, list) else (
            t2_data.get("data")
        ...
        assert not overlap, (
            f"Cross-tenant employee leakage detected: {overlap}"
        )
