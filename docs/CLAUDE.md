...
cp .env.example .env
docker-compose up -d --build        # http://localhost (port 80)
...
- **Database**: PostgreSQL 15, 8 tables with `updated_at` triggers, `init-db.sql` seed script. No Alembic migrations yet — tables created via `async_init_db()` on startup.
