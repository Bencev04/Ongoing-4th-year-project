...
class TestListJobs:
    ...
    def test_list_jobs_requires_auth(self, http_client: httpx.Client) -> None:
        """
...
class TestCalendarEndpoint:
    ...
    def test_calendar_returns_day_structure(
        self,
        http_client: httpx.Client,
        owner_headers: dict[str, str],
    ) -> None:
        """
        Test the calendar response contains correctly structured day entries.

        Verifies:
        - Response is a list of day objects
        - Each day has 'date', 'jobs', and 'total_jobs' fields
        - Number of days matches the requested range
        """
        resp = http_client.get(
            "/api/v1/jobs/calendar",
            headers=owner_headers,
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert isinstance(data, list), "Calendar should return a list"
        assert len(data) == 7, f"Expected 7 days, got {len(data)}"
        for day in data:
            assert "date" in day, "Day entry missing 'date'"
            assert "jobs" in day, "Day entry missing 'jobs'"
            assert "total_jobs" in day, "Day entry missing 'total_jobs'"

    def test_multi_day_job_appears_on_all_spanned_dates(
        self,
        http_client: httpx.Client,
        owner_headers: dict[str, str],
    ) -> None:
        """
        Test that a job spanning multiple days appears on every date.

        Steps:
        1. Create a job scheduled from Mar 10 to Mar 12 (3 days)
        2. Query the calendar for Mar 9–13 (5 days)
        3. Verify the job appears on Mar 10, 11, AND 12
        4. Verify the job does NOT appear on Mar 9 or 13

        Verifies:
        - Multi-day job expansion works in the BL service
        - The same job ID appears on each spanned date
        """
        ...
        create_resp = http_client.post(
            ...
            headers=owner_headers,
            json={
                "title": "Multi-Day Integration Test",
                "description": "Spans 3 days for calendar expansion test.",
                "status": "scheduled",
                "priority": "normal",
                "start_time": start,
                "end_time": end,
        ...
        assert create_resp.status_code in (200, 201), (
            f"Job creation failed: {create_resp.status_code} — {create_resp.text}"
        )
        job = create_resp.json()
        job_id = job.get("id") or job.get("job_id")

        try:
            # Query the calendar for 5 days surrounding the job
            ...
            # Build a date → job_ids lookup
            date_jobs: dict[str, list] = {}
            for day in days:
                day_date = day.get("date", "")
                # date may be a date object serialized as "2026-03-09"
                day_str = str(day_date)[:10]
                day_job_ids = [
                    j.get("id") or j.get("job_id") for j in day.get("jobs", [])
                ]
                date_jobs[day_str] = day_job_ids

            # Job should appear on Mar 10, 11, 12
            for expected_date in ("2026-03-10", "2026-03-11", "2026-03-12"):
                assert job_id in date_jobs.get(expected_date, []), (
                    f"Multi-day job {job_id} missing from {expected_date}. "
                    f"Found: {date_jobs.get(expected_date, [])}"
                )

            # Job should NOT appear on Mar 9 or 13
            for excluded_date in ("2026-03-09", "2026-03-13"):
                assert job_id not in date_jobs.get(excluded_date, []), (
                    f"Multi-day job {job_id} should not appear on {excluded_date}"
                )
        finally:
            # Cleanup
            http_client.delete(
    ...
    def test_calendar_requires_auth(self, http_client: httpx.Client) -> None:
...
class TestQueueEndpoint:
    ...
    def test_queue_requires_auth(self, http_client: httpx.Client) -> None:
...
class TestJobCRUD:
    ...
    def _get_first_customer_id(
        ...
        customers = (
            ...
            else (data.get("data") or data.get("items") or data.get("customers", []))
        )
        if not customers:
            return None
