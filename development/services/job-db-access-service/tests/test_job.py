...
from app.schemas import JobCreate, JobStatusUpdate, JobUpdate
...
class TestJobCRUD:
    ...
    async def test_complete_job_with_duration(
        ...
        status_data = JobStatusUpdate(status=JobStatus.COMPLETED, actual_duration=180)
        updated = await update_job_status(
            db_session, job.id, status_data, changed_by_id=1
        )
...
class TestJobAPI:
    ...
    async def test_health_check(self, client: AsyncClient) -> None:
        ...
        assert data["service"] == "job-db-access-service"
    ...
    async def test_list_jobs_without_owner_id_returns_all(
...
class TestJobDateTimeHandling:
    ...
    async def test_job_duration_calculation_accuracy(
        ...
        status_update = JobStatusUpdate(status=JobStatus.COMPLETED, actual_duration=210)
    ...
    async def test_jobs_ordered_by_start_time(
        ...
        jobs, _ = await get_jobs(db_session, owner_id=1, status=[JobStatus.SCHEDULED])
...
class TestJobStatusLifecycle:
    ...
    async def test_pending_to_scheduled_on_scheduling(
        ...
        job = await create_job(db_session, JobCreate(**sample_unscheduled_job_data))
        assert job.status == JobStatus.PENDING

        now = datetime.utcnow()
        update = JobUpdate(start_time=now, end_time=now + timedelta(hours=2))
...
class TestJobIncludeHistory:
    ...
    async def test_get_job_with_history_flag(
        ...
        response = await client.get(f"/api/v1/jobs/{job_id}?include_history=true")
        assert response.status_code == 200
...
class TestJobAutoStatusPromotion:
    ...
    async def test_pending_without_start_time_stays_pending_via_api(
        ...
        """
        response = await client.post("/api/v1/jobs", json=sample_unscheduled_job_data)
        assert response.status_code == 201
        assert response.json()["status"] == "pending"


# ==============================================================================
# Job-Employee Assignment Tests
# ==============================================================================


class TestJobEmployeeAssignment:
    """Tests for job-employee assignment endpoints."""

    async def test_assign_employee_to_job(
        self, client: AsyncClient, sample_job_data: dict
    ) -> None:
        """POST /jobs/{id}/employees returns 201 with assignment details."""
        job_resp = await client.post("/api/v1/jobs", json=sample_job_data)
        job_id = job_resp.json()["id"]

        response = await client.post(
            f"/api/v1/jobs/{job_id}/employees",
            json={"employee_id": 10, "owner_id": 1},
        )
        assert response.status_code == 201
        body = response.json()
    ...
    async def test_assign_employee_to_nonexistent_job(
        self, client: AsyncClient
    ) -> None:
        """POST /jobs/99999/employees returns 404 when job doesn't exist."""
        response = await client.post(
            "/api/v1/jobs/99999/employees",
            json={"employee_id": 10, "owner_id": 1},
        )
        assert response.status_code == 404
        assert "Job not found" in response.json()["detail"]

    async def test_list_job_employees(
        self, client: AsyncClient, sample_job_data: dict
    ) -> None:
        """GET /jobs/{id}/employees returns list of assigned employee IDs."""
        job_resp = await client.post("/api/v1/jobs", json=sample_job_data)
        job_id = job_resp.json()["id"]
        ...
        await client.post(
            f"/api/v1/jobs/{job_id}/employees",
            json={"employee_id": 20, "owner_id": 1},
        )

        response = await client.get(f"/api/v1/jobs/{job_id}/employees")
        assert response.status_code == 200
        employee_ids = response.json()
        assert set(employee_ids) == {10, 20}

    async def test_list_job_employees_empty(
        self, client: AsyncClient, sample_job_data: dict
    ) -> None:
        """GET /jobs/{id}/employees returns empty list when no assignments."""
        job_resp = await client.post("/api/v1/jobs", json=sample_job_data)
        job_id = job_resp.json()["id"]
    ...
    async def test_unassign_employee_from_job(
        self, client: AsyncClient, sample_job_data: dict
    ) -> None:
        """DELETE /jobs/{id}/employees/{eid} returns 204 and removes assignment."""
        job_resp = await client.post("/api/v1/jobs", json=sample_job_data)
        job_id = job_resp.json()["id"]
        ...
        response = await client.delete(f"/api/v1/jobs/{job_id}/employees/10")
        assert response.status_code == 204

        list_resp = await client.get(f"/api/v1/jobs/{job_id}/employees")
        assert 10 not in list_resp.json()

    async def test_unassign_nonexistent_assignment(
        self, client: AsyncClient, sample_job_data: dict
    ) -> None:
        """DELETE /jobs/{id}/employees/{eid} returns 404 when not assigned."""
        job_resp = await client.post("/api/v1/jobs", json=sample_job_data)
        job_id = job_resp.json()["id"]

        response = await client.delete(f"/api/v1/jobs/{job_id}/employees/999")
        assert response.status_code == 404
