...
def _mock_permission_http_check():
    ...
    ``require_permission`` calls user-db-access-service via ``httpx.AsyncClient.get``
    to verify fine-grained permissions.  In the test environment there is no
    running service, so we patch the class-level ``get`` method to return
    a realistic response based on the employee role defaults.

    Privileged roles (owner, admin, superadmin) bypass the HTTP call entirely,
    so this fixture has no effect on owner/admin test clients.
    """
