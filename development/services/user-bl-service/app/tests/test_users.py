...
class TestCreateEmployee:
    ...
    def test_owner_can_create_employee(
        ...
        """Owner creates an employee record successfully."""
        mock_create.return_value = _SAMPLE_EMPLOYEE
        response = owner_client.post(
            "/api/v1/employees",
            json={"user_id": 10, "position": "Plumber", "hourly_rate": 25.0},
        )
        assert response.status_code == 201
        assert response.json()["position"] == "Plumber"
    ...
    def test_employee_cannot_create_employee(
        ...
        """Employee role lacks employees.create permission → 403."""
        response = employee_client.post(
            "/api/v1/employees",
            json={"user_id": 10, "position": "Plumber"},
        )
        assert response.status_code == 403
...
class TestGetEmployee:
    ...
    @patch("app.service_client.get_employee", new_callable=AsyncMock)
    def test_owner_can_get_employee(
        self,
        mock_get: AsyncMock,
        owner_client: TestClient,
    ) -> None:
        """Owner retrieves employee in same tenant."""
        mock_get.return_value = _SAMPLE_EMPLOYEE
        response = owner_client.get("/api/v1/employees/5")
        assert response.status_code == 200
        assert response.json()["id"] == 5

    @patch("app.service_client.get_employee", new_callable=AsyncMock)
    def test_cross_tenant_employee_access_denied(
        self,
        mock_get: AsyncMock,
        owner_client: TestClient,
    ) -> None:
        """Owner cannot access employee from a different tenant → 403."""
        mock_get.return_value = {**_SAMPLE_EMPLOYEE, "owner_id": 999}
        response = owner_client.get("/api/v1/employees/5")
        assert response.status_code == 403


class TestUpdateEmployee:
    """Tests for PUT /api/v1/employees/{id}."""

    @patch("app.service_client.update_employee", new_callable=AsyncMock)
    @patch("app.service_client.get_employee", new_callable=AsyncMock)
    def test_owner_can_update_employee(
        self,
        mock_get: AsyncMock,
        mock_update: AsyncMock,
        owner_client: TestClient,
    ) -> None:
        """Owner updates an employee in same tenant."""
        mock_get.return_value = _SAMPLE_EMPLOYEE
        mock_update.return_value = {**_SAMPLE_EMPLOYEE, "position": "Senior Plumber"}
        response = owner_client.put(
            "/api/v1/employees/5",
    ...
    def test_employee_cannot_update_employee(
        ...
        """Employee role lacks employees.edit permission → 403."""
        response = employee_client.put(
            "/api/v1/employees/5",
...
class TestDeleteEmployee:
    """Tests for DELETE /api/v1/employees/{id}."""
    ...
    def test_owner_can_delete_employee(
        ...
        """Owner soft-deletes employee's linked user account."""
        mock_get.return_value = _SAMPLE_EMPLOYEE
        mock_delete.return_value = None
        response = owner_client.delete("/api/v1/employees/5")
        assert response.status_code == 204
        mock_delete.assert_called_once_with(10)  # user_id from employee
    ...
    def test_owner_cannot_delete_self(
        ...
        """Owner cannot deactivate their own account → 400."""
        mock_get.return_value = {
            **_SAMPLE_EMPLOYEE,
            "user_id": 1,
        }  # matches owner user_id
        response = owner_client.delete("/api/v1/employees/5")
        assert response.status_code == 400
    ...
    def test_employee_cannot_delete_employee(
        ...
        """Employee role lacks employees.delete permission → 403."""
        response = employee_client.delete("/api/v1/employees/5")
        assert response.status_code == 403
