...
async def store_refresh_token(
    ...
    expires_at: datetime = datetime.now(UTC) + timedelta(days=expires_days)
...
async def is_token_blacklisted(
    ...
    # Fail-closed: if neither Redis nor Postgres is reachable we must
    # assume the token *could* be blacklisted.  Letting a revoked token
    # through is far worse than temporarily rejecting a valid one.
    logger.warning("Cannot verify blacklist for jti=%s — failing closed", jti)
    return True
