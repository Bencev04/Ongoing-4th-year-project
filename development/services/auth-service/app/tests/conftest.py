...
# ---------------------------------------------------------------------------
# FakeRedis — in-memory dict-backed async Redis substitute
# ---------------------------------------------------------------------------
...
class FakeRedis:
    ...
    Supports the subset of commands used by the auth CRUD layer:
    ``setex``, ``exists``, and ``get``.  TTL is ignored (tests are
    short-lived so expiry is irrelevant).
    """
...
def _disable_rate_limiter() -> None:
    """Disable request-rate limiting during tests for deterministic behavior."""
...
@pytest.fixture(autouse=True)
def _mock_redis():
    """Patch ``get_redis`` with a FakeRedis instance.

    This allows tests to exercise the Redis fast-path in
    ``is_token_blacklisted`` and ``blacklist_access_token`` without
    a real Redis server.  The store is cleared between tests.
    """
    _fake_redis_instance.store.clear()

    async def _get_fake_redis():
        return _fake_redis_instance

    with patch("app.crud.auth.get_redis", new=_get_fake_redis):
        yield _fake_redis_instance


@pytest.fixture()
def redis_unavailable():
    """Override ``_mock_redis`` to simulate Redis being down.

    Use this fixture in tests that must verify the Postgres fallback
    path.  Import it explicitly in the test parameter list.
    """

    async def _fail():
        raise Exception("Redis unavailable in tests")
