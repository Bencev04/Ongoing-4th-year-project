...
from app.schemas import CustomerCreate, CustomerNoteCreate, CustomerUpdate
...
class TestCustomerCRUD:
    ...
    async def test_get_customers_with_search(
        ...
        customers, total = await get_customers(db_session, owner_id=1, search="John")
...
class TestCustomerAPI:
    ...
    async def test_health_check(self, client: AsyncClient) -> None:
        ...
        assert data["service"] == "customer-db-access-service"
    ...
    async def test_get_customer_endpoint(
        ...
        create_resp = await client.post("/api/v1/customers", json=sample_customer_data)
    ...
    async def test_list_customers_requires_owner_id(self, client: AsyncClient) -> None:
...
class TestCustomerSearchAndFiltering:
    ...
    async def test_search_customer_by_email(
        ...
        results, _ = await get_customers(db_session, owner_id=1, search="unique.email")
    ...
    async def test_search_customer_by_phone(
        ...
        await create_customer(
            db_session,
            CustomerCreate(**{**sample_customer_data, "phone": "0871234567"}),
        )
    ...
    async def test_search_respects_tenant_isolation(
        ...
        results, total = await get_customers(db_session, owner_id=1, search="Customer")

        assert total == 1
        assert len(results) == 1
        assert all(c.owner_id == 1 for c in results)
        assert results[0].name == "Tenant 1 Customer"
...
class TestCustomerNoteManagement:
    ...
    async def test_notes_ordered_by_creation_time(
        ...
        assert total == 3
        assert len(notes) == 3
        # Verify descending order: newest (Note 2) first, oldest (Note 0) last
        assert notes[0].content == "Note 2"
        assert notes[1].content == "Note 1"
        assert notes[2].content == "Note 0"
...
class TestCustomerDataIntegrity:
    ...
    async def test_customer_email_uniqueness_per_tenant(
        ...
        customer2 = await create_customer(db_session, CustomerCreate(**data_tenant2))
    ...
    async def test_customer_address_fields_optional(
        ...
        customer = await create_customer(db_session, CustomerCreate(**minimal_data))
...
class TestCustomerSoftDeleteAndReactivation:
    ...
    async def test_deactivated_customer_not_in_active_listings(
        ...
        results, _ = await get_customers(db_session, owner_id=1, is_active=True)
...
class TestCustomer404Paths:
    ...
    async def test_update_nonexistent_customer_returns_404(
        ...
        response = await client.put("/api/v1/customers/99999", json={"name": "Ghost"})
