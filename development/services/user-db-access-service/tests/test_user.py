...
from app.schemas import EmployeeCreate, UserCreate, UserUpdate
...
class TestEmployeeCRUD:
    ...
    async def test_create_employee(
        ...
        emp = await create_employee(
            db_session,
            EmployeeCreate(user_id=user.id, owner_id=owner.id, **sample_employee_data),
        )
...
class TestUserAPI:
    ...
    async def test_health_check(self, client: AsyncClient) -> None:
        ...
        assert data["service"] == "user-db-access-service"
...
class TestEmployeeAPI:
    ...
    async def test_create_employee_endpoint(
        ...
        user_resp = await client.post(
            "/api/v1/users", json={**sample_user_data, "owner_id": owner_id}
        )
...
class TestUserRolesAndPermissions:
    ...
    async def test_user_role_cannot_be_invalid(self, client: AsyncClient) -> None:
        response = await client.post(
            "/api/v1/users",
            json={
                "email": "test@test.com",
...
class TestUserLifecycleManagement:
    ...
    async def test_reactivate_deactivated_user(
        ...
        user = await create_user(db_session, UserCreate(**sample_user_data))
        await delete_user(db_session, user.id)
        reactivated = await update_user(db_session, user.id, UserUpdate(is_active=True))
...
class TestCompanyCRUD:
    ...
    async def test_create_company(self, db_session: AsyncSession) -> None:
        ...
        company = await create_company(
            db_session,
            CompanyCreate(
                name="Acme Plumbing", phone="0851234567", email="info@acme.ie"
            ),
        )
    ...
    async def test_get_company(self, db_session: AsyncSession) -> None:
        ...
        created = await create_company(db_session, CompanyCreate(name="Test Co"))
...
class TestCompanyAPI:
    ...
    async def test_update_nonexistent_company_returns_404(
        ...
        response = await client.put("/api/v1/companies/99999", json={"name": "Nope"})
        assert response.status_code == 404
...
class TestInternalAuthentication:
    ...
    async def test_authenticate_nonexistent_email(self, client: AsyncClient) -> None:
...
class TestUserAPI404Paths:
    ...
    async def test_put_nonexistent_user_returns_404(self, client: AsyncClient) -> None:
...
class TestOrganizationAPI:
    ...
    async def test_unsuspend_organization(self, client: AsyncClient) -> None:
        ...
        response = await client.post(f"/api/v1/organizations/{org_id}/unsuspend")
        assert response.status_code == 200
        data = response.json()
        assert data["is_active"] is True
...
class TestAuditLogAPI:
    ...
    async def test_list_audit_logs_filter_by_action(self, client: AsyncClient) -> None:
...
class TestPlatformSettingsAPI:
    ...
    async def test_list_platform_settings_empty(self, client: AsyncClient) -> None:
    ...
    async def test_upsert_creates_new_setting(self, client: AsyncClient) -> None:
    ...
    async def test_upsert_updates_existing_setting(self, client: AsyncClient) -> None:
    ...
    async def test_get_platform_setting_by_key(self, client: AsyncClient) -> None:
...
class TestUserListPagePerPage:
    ...
    async def test_page_two(self, client: AsyncClient, sample_user_data: dict) -> None:
...
class TestEmployeesByOwnerEndpoint:
    ...
    async def test_list_employees_returns_paginated_response(
        self,
        client: AsyncClient,
        db_session: AsyncSession,
        sample_owner_data: dict,
        ...
        """GET /users/{id}/employees returns paginated employee list."""
        owner_resp = await client.post("/api/v1/users", json=sample_owner_data)
        owner_id = owner_resp.json()["id"]

        user_data = {**sample_user_data, "owner_id": owner_id}
        user_resp = await client.post("/api/v1/users", json=user_data)
        user_id = user_resp.json()["id"]

        emp_data = {**sample_employee_data, "user_id": user_id, "owner_id": owner_id}
        await client.post("/api/v1/employees", json=emp_data)
    ...
    async def test_list_employees_empty(
        ...
        """GET /users/{id}/employees returns empty list when no employees."""
        owner_resp = await client.post("/api/v1/users", json=sample_owner_data)
        owner_id = owner_resp.json()["id"]
    ...
    async def test_list_employees_with_search(
        ...
        """GET /users/{id}/employees?search=X filters by name/email/position."""
        owner_resp = await client.post("/api/v1/users", json=sample_owner_data)
        owner_id = owner_resp.json()["id"]
        ...
        response = await client.get(
            f"/api/v1/users/{owner_id}/employees",
            params={"search": "Plumber"},
        )
        assert response.status_code == 200
        body = response.json()
        assert body["total"] >= 1
    ...
    async def test_list_employees_pagination(
        ...
        """GET /users/{id}/employees respects skip and limit params."""
        owner_resp = await client.post("/api/v1/users", json=sample_owner_data)
        owner_id = owner_resp.json()["id"]
        ...
        for i in range(3):
            ...
            emp_data = {**sample_employee_data, "user_id": uid, "owner_id": owner_id}
            await client.post("/api/v1/employees", json=emp_data)
...
class TestAuditLogPagination:
    ...
    async def test_audit_logs_filter_by_actor_id(self, client: AsyncClient) -> None:
...
class TestOrganizationPagination:
    ...
    async def test_organization_list_page_per_page(self, client: AsyncClient) -> None:
